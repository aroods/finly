...
def yahoo_search():
    ...
    try:
        ...
        for item in resp.json().get("quotes", []):
            # Only include stocks/etfs with a symbol and shortname
...
def fetch_current_prices():
    ...
    for asset, currency, category, total_qty in rows:
        print(f"Fetching: {asset}, Currency: {currency}, Category: {category}, Quantity: {total_qty}")
        ...
        print(f"Ticker: {asset} | Price: {price}")
        ...
        print(f"FX Rate {currency} to PLN: {currency_rates[currency]}")
...
def edit_transaction(tx_id):
    ...
    if request.method == 'POST':
        ...
        ttype = request.form.get('type').lower()
        quantity = request.form.get('quantity')
        price = request.form.get('price')
        currency = request.form.get('currency').strip().upper()
        category = request.form.get('category').strip()
        cur.execute("""
            UPDATE transactions SET date=?, asset=?, type=?, quantity=?, price=?, currency=?, category=?
            WHERE id=?
        """, (date, asset, ttype, float(quantity), float(price), currency, category, tx_id))
        conn.commit()
